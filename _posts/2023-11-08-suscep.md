...
We propose a computational model to infer a user's unobservable susceptibility score based on their historical activities (Section \ref{sec:sus_model}) as shown in Figure \ref{fig:model}, and further train the model with signals from the observable disseminating behavior (Section \ref{sec:training}). We construct approximate contrastive user-post pairs as the training data (Section \ref{sec:data_training}).
...
Given the input of the user profile for the user $u$ and the content for misinformation post $p$, the susceptibility computational model is expected to produce the _susceptibility score_ $s_{u, p}$ as shown in Equation \ref{eq:sus}, reflecting the susceptibility of $u$ when $p$ is perceived.
...
```latex
\begin{equation}\label{eq:retweet}
...
\mathcal{L}_{\text{triplet}}(u_a, u_s, u_{ds}, t) &= \text{ReLU}(\Vert s_{u_{a},t} - s_{u_{s},t}\Vert_2^2 - \Vert s_{u_{a},t} - s_{u_{ds},t} \Vert_2^2 + \alpha) \nonumber \\
\mathcal{L}(u_a, u_s, u_{ds}, t) &= \frac{\lambda}{3} \sum_{i \in \{a, s, ds\}} \mathcal{L}_{\text{bce}}(u_i, t) + (1 - \lambda) \mathcal{L}_{\text{triplet}}(u_a, u_s, u_{ds}, t) \label{eq:loss}
\end{align}
\label{fig:loss}
\end{figure*}

#### Objectives
We perform multi-task learning to utilize different supervision signals. We first consider a binary classification task of predicting repost or not with a cross-entropy loss. Additionally, we perform the triplet ranking task~\cite{chen2009ranking,hoffer2015deep} to distinguish the subtle differences among the susceptibility scores of multiple users when the same false content is present.
