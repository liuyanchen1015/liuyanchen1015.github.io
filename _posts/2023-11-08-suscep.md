...
_Baseline_ refers to a direct comparison of cosine similarity, while _ChatGPT_ prompts the ChatGPT model (engine gpt-3.5-turbo) to predict the more susceptible user.
The 95% confidence intervals of the agreements are determined through a paired bootstrap test. A t-test comparing the agreement rates of Our method and the Baseline demonstrates a highly statistically significant difference (p = 3e-15).
